"""
為 USR_TOPIC_KEYWORDS 每個類別自動生成擴充關鍵詞，並直接寫入 app.py。
只需跑一次。
"""
import os, json, re
...
PROMPT = """你是一位熟悉台灣大學社會責任（USR）計畫的專家。

以下是「{topic}」類別目前已有的關鍵詞：
{existing}

請針對這個類別，補充 10～20 個**目前清單中沒有**的相關關鍵詞。
要求：
- 這些詞應該是台灣 USR 計畫書中可能實際出現的詞彙
- 涵蓋同義詞、常用別稱、相關活動名稱、延伸概念
- 每個詞 2～8 個字，純中文（英文縮寫除外）
- 只輸出 JSON 陣列格式，不要其他說明
...
for topic, existing_kws in USR_TOPIC_KEYWORDS.items():
    print(f"處理：{topic} ...")
    prompt = PROMPT.format(topic=topic, existing="、".join(existing_kws))
    response = llm.invoke(prompt)
    content = response.content
    if isinstance(content, list):
        raw = "".join(b.get("text", "") if isinstance(b, dict) else str(b) for b in content).strip()
    else:
        raw = content.strip()
    ...
    if match:
        try:
            new_kws = json.loads(match.group())
            new_kws = [k for k in new_kws if k not in existing_kws]
            expanded[topic] = new_kws
            print(f"  → 新增 {len(new_kws)} 個：{new_kws}")
...
for topic, new_kws in expanded.items():
    if not new_kws:
        continue

    # 找到該 topic 區塊的最後一個關鍵詞行（],）並在其前插入新詞
    # 格式：    "xxx", "yyy",\n    ],
    pattern = rf'("{re.escape(topic)}":\s*\[.*?)\n(\s*\],)'
    match = re.search(pattern, content, re.DOTALL)
    if not match:
        print(f"  !! 找不到 {topic} 區塊，略過")
        continue

    new_line = "        " + ", ".join(f'"{k}"' for k in new_kws) + ","
    replacement = match.group(1) + "\n" + new_line + "\n" + match.group(2)
    content = content[:match.start()] + replacement + content[match.end():]
    print(f"已寫入「{topic}」{len(new_kws)} 個詞")

app_path.write_text(content, encoding="utf-8")
print("\n完成！已更新 app.py，請重啟系統讓倒排索引重建。")
