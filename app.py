...
【回答前請先判斷】
請先理解使用者問的精確意圖，再判斷 context 是否能回答：
- 若使用者問題含有「已完成」「完成了」「結案」等詞，必須在 context 中找到明確的完成陳述才算能回答；若 context 只有「正在進行」「初步估算」「導入中」「規劃中」等進行中描述，視為無法回答。
- 若 context 內容與問題無關，或只有個案描述而問題問的是普遍規則，視為無法回答。
