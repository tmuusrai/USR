...
USR_TOPIC_KEYWORDS: dict[str, list[str]] = {
    ...
    "健康促進與食品安全": [
        ...
        "延緩失能", "活躍老化", "產銷履歷", "友善農法", "惜食教育", "園藝治療", "高齡體適能", "智慧健康", "低碳飲食", "食品檢驗", "社區健康營造", "綠色餐飲", "長照服務", "全人健康", "有機農業", "遠距照護", "食安把關", "營養介入",
        "食育", "地產地消", "飲食教育", "食農體驗", "農業體驗", "農耕教育", "農耕體驗",
        "有機栽培", "農業推廣", "社區農園", "農夫市集", "小農市集", "校園農場",
        "食品衛生", "飲食安全", "農業永續", "農業文化", "食材教育", "農業知識",
        "長照", "樂齡", "銀髮族", "失智症", "長者", "高齡友善", "健康老化", "照顧服務",
        "居家照顧", "社區照顧", "日間照顧", "失能預防", "銀髮", "老人健康", "老化",
        "高齡化", "失智", "長期照顧", "家庭照顧者", "照護服務", "健康促進站",
    ],
    "產業鏈結與經濟永續": [
        "地方創生", "產學合作", "青年返鄉", "產業升級", "產業鏈結", "在地產業",
        "數位轉型", "社會企業", "微型創業", "品牌行銷", "農業加值", "創新創業",
        "永續商業模式", "人才培育", "就業媒合", "區域經濟", "在地經濟", "技術轉移",
        "在地產品", "產業活化", "就業輔導", "產業轉型", "企業合作", "企業ESG合作",
        "永續經濟", "場域經營",
        "循環經濟", "六級產業化", "社會創新", "地方品牌", "體驗經濟", "跨域合作", "智慧農業", "綠色經濟", "數位行銷", "青年培力", "創生基地", "產銷共好", "資源整合", "電商平台", "社區經濟", "價值鏈整合", "科技導入", "地方小旅行", "綠色消費", "農產品開發",
    ],
    "文化永續": [
        ...
        "文創設計", "部落共學",
        "文化轉譯", "文化創生", "客家文化", "眷村文化", "新住民文化", "母語復振", "地方文化館", "民俗節慶", "聚落保存", "產業遺產", "文化地景", "飲食文化", "地方誌編纂", "藝術介入空間", "文化平權", "傳統工藝", "生態博物館", "歷史記憶", "祭典傳承", "匠師技藝",
    ],
    "其他社會實踐": [
        "社會創新", "教育平權", "數位平權", "公民參與", "多元文化", "性別平等",
        "防災教育", "社區安全", "法律扶助", "科技導入", "智慧社區", "新住民支持",
        "身心障礙者支持", "動物保護", "動物福祉", "流浪動物", "動物收容", "TNR", "絕育", "寵物友善", "人犬關係", "人權教育", "社會共融", "國際合作", "特殊議題",
        "政策倡議", "減少不平等",
        "弱勢關懷", "偏鄉教育", "青銀共創", "社會企業", "移工關懷", "媒體識讀", "社區營造", "培力賦能", "服務學習", "社會設計", "無家者關懷", "勞動權益", "代間學習", "社會安全網", "普惠金融", "社會倡議", "高齡友善", "韌性社區", "跨文化交流", "志願服務",
    ],
    "計畫行政": [
    ...
    def generate():
        try:
            ...
            if annotated and _list:
                ...
                # 地區過濾：問題含地區關鍵字時只保留該地區學校
                _question_region = _detect_question_region(question)
                if _question_region:
                    _region_filtered = [l for l in _plan_list_lines if _get_school_region(l.split('：')[0]) == _question_region]
                    if _region_filtered:
                        _plan_list_lines = _region_filtered
                        print(f"[LIST-REGION] 地區過濾={_question_region}，剩餘 {len(_plan_list_lines)} 件")
                    else:
                        print(f"[LIST-REGION] 地區過濾={_question_region}，無匹配學校，不過濾")
                ...
                def _sum_one_plan(_plan_line: str) -> str:
                    ...
                    _p = (
                        ...
                        f"- 若內容僅含章節標題（如「一、」「（一）」「叁、」「## 標題」等）或單位名稱清單、聯絡表格等無具體描述文字，直接輸出「#RAW」\n"
                        f"只輸出擷取的句子，不要其他文字。\n\n{_snip}"
                    )
                    try:
...
_LIST_CONCEPT_RE = re.compile(
    ...
    r'|各.{0,6}(?:校|學校|大學|院校).{0,20}(?:如何|以何|方式|做法|程序|機制|策略|方法|途徑|模式|作法|進行|實施|推動)'
)
...
# ── 學校地區對照表 ──
_SCHOOL_REGION: dict[str, str] = {
    # 北部（台北・新北・基隆・桃園・新竹・宜蘭）
    '大同大學': '北部', '中國文化大學': '北部', '中國科技大學': '北部',
    '中華大學': '北部', '明志科技大學': '北部', '明新科技大學': '北部',
    '東吳大學': '北部', '東南科技大學': '北部', '法鼓文理學院': '北部',
    '長庚大學': '北部', '長庚科技大學': '北部', '致理科技大學': '北部',
    '耕莘健康管理專科學校': '北部', '馬偕醫學大學': '北部', '馬偕醫護管理專科學校': '北部',
    '健行科技大學': '北部', '國立中央大學': '北部', '國立宜蘭大學': '北部',
    '國立陽明交通大學': '北部', '國立清華大學': '北部', '國立臺北大學': '北部',
    '國立臺北科技大學': '北部', '國立臺北護理健康大學': '北部',
    '國立臺灣大學': '北部', '國立臺灣科技大學': '北部', '國立臺灣師範大學': '北部',
    '國立臺灣海洋大學': '北部', '台北海洋科技大學': '北部', '亞東科技大學': '北部',
    '元智大學': '北部', '元培醫事科技大學': '北部', '中原大學': '北部',
    '玄奘大學': '北部', '佛光大學': '北部', '開南大學': '北部',
    '淡江大學': '北部', '華梵大學': '北部', '輔仁大學': '北部',
    '銘傳大學': '北部', '德明財經科技大學': '北部', '龍華科技大學': '北部',
    '臺北市立大學': '北部', '臺北醫學大學': '北部', '敏實科技大學': '北部',
    '萬能科技大學': '北部', '實踐大學': '北部', '經國管理學院': '北部',
    '德育護理健康學院': '北部',
    # 中部（苗栗・台中・彰化・南投・雲林）
    '大葉大學': '中部', '中臺科技大學': '中部', '中國醫藥大學': '中部',
    '弘光科技大學': '中部', '東海大學': '中部', '逢甲大學': '中部',
    '朝陽科技大學': '中部', '靜宜大學': '中部', '嶺東科技大學': '中部',
    '亞洲大學': '中部', '育達科技大學': '中部', '僑光科技大學': '中部',
    '國立中興大學': '中部', '國立虎尾科技大學': '中部', '國立雲林科技大學': '中部',
    '國立勤益科技大學': '中部', '國立彰化師範大學': '中部', '國立暨南國際大學': '中部',
    '國立臺中科技大學': '中部', '國立臺中教育大學': '中部', '建國科技大學': '中部',
    '南開科技大學': '中部', '國立聯合大學': '中部',
    # 南部（嘉義・台南・高雄・屏東）
    '大仁科技大學': '南部', '中華醫事科技大學': '南部', '中信科技大學': '南部',
    '文藻外語大學': '南部', '台南應用科技大學': '南部', '正修科技大學': '南部',
    '長榮大學': '南部', '南華大學': '南部', '南臺科技大學': '南部',
    '美和科技大學': '南部', '高雄醫學大學': '南部', '國立中山大學': '南部',
    '國立中正大學': '南部', '國立成功大學': '南部', '國立屏東大學': '南部',
    '國立屏東科技大學': '南部', '國立高雄大學': '南部', '國立高雄科技大學': '南部',
    '國立高雄師範大學': '南部', '國立高雄餐旅大學': '南部', '國立嘉義大學': '南部',
    '國立臺南大學': '南部', '國立臺南藝術大學': '南部', '崑山科技大學': '南部',
    '嘉南藥理大學': '南部', '樹德科技大學': '南部', '義守大學': '南部',
    '輔英科技大學': '南部',
    # 東部（花蓮・台東）
    '國立東華大學': '東部', '國立臺東大學': '東部', '慈濟大學': '東部',
    # 離島（金門・澎湖）
    '國立金門大學': '離島', '國立澎湖科技大學': '離島',
}

_REGION_KEYWORDS: dict[str, list[str]] = {
    '北部': ['北部', '北台灣', '北臺灣', '台北', '臺北', '新北', '基隆', '桃園', '新竹', '宜蘭'],
    '中部': ['中部', '中台灣', '中臺灣', '台中', '臺中', '彰化', '南投', '雲林', '苗栗'],
    '南部': ['南部', '南台灣', '南臺灣', '台南', '臺南', '高雄', '屏東', '嘉義'],
    '東部': ['東部', '東台灣', '東臺灣', '花蓮', '台東', '臺東'],
    '離島': ['離島', '金門', '澎湖', '馬祖'],
}

def _get_school_region(school_name: str) -> str:
    if school_name in _SCHOOL_REGION:
        return _SCHOOL_REGION[school_name]
    for k, v in _SCHOOL_REGION.items():
