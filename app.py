...
【回答前請先判斷】
請先理解使用者問題的精確意圖（例如：問的是完成狀態、進行中狀態、數量、原因等），再確認 context 中的內容是否對應到該意圖。若 context 描述的狀態或內容與使用者問的不一致（例如：使用者問完成，但 context 只有進行中的描述），視為無法回答。
特別注意：若使用者詢問「是否需要」「是否必須」「規定上要不要」等規定性問題，context 中某計畫「有做過」某件事，不能作為該事項「有規定需要這樣做」的依據，應視為無法回答。
無法回答時，請直接輸出以下文字，不得加任何前言或解釋：
「本系統僅收錄 USR 計畫成果報告，目前資料庫中沒有關於這個問題的相關資料，建議參考其他管道查詢。」
若計畫書內容能直接回答，則照以下規則回答。
...
- 凡提及計畫，格式必須為「學校全名：計畫全名」，例：國立成功大學：城鄉相伴健康永續生活，不得省略學校名稱或只寫縮寫。學校全名包含「國立」「私立」等前綴，例如必須寫「國立中興大學」不得縮寫為「中興大學」。
...
- 本系統僅收錄 USR 計畫成果報告；若問題與計畫書內容完全無關，請直接告知「本系統僅收錄 USR 計畫成果報告，目前資料庫中沒有相關資料。」
- 回答請使用繁體中文。
...
_KW_EXTRA_LOOKUP: dict[str, list[str]] = {
    "原住民":   ["原住民族關懷", "原鄉教育", "原住民族文化", "原住民族研究", "原鄉醫療", "部落共學"],
    "原住民族": ["原住民族關懷", "原鄉教育", "原住民族文化", "原住民族研究", "原鄉醫療", "部落共學"],
    "原民":     ["原住民族關懷", "原鄉教育", "原住民族文化", "原住民族研究", "原鄉醫療", "部落共學"],
    "原鄉":     ["原鄉教育", "原鄉醫療", "原住民族關懷", "部落共學"],
    "部落":     ["原住民族關懷", "原鄉教育", "部落共學", "原住民族文化"],
    "族語":     ["原住民族關懷", "原住民族文化", "原住民族研究"],
    "高齡":     ["高齡照護", "青銀共創", "銀髮族"],
    "長者":     ["高齡照護", "青銀共創"],
    "長照":     ["高齡照護"],
    "失智":     ["高齡照護"],
        ...
        try:
            ...
            if _eval_criterion:
                context = (
                    ...
                    f"說明其具體內容，最多只列出 2 所最具代表性的學校。沒有相關內容的計畫不要列出。\n\n"
                ) + context
...
請判斷以下問題**最核心**屬於哪個 USR 議題類別。

規則：
- 只選與問題**最核心、最直接**相關的一個類別
- 只能選 1 個，不得複選
- 若問題與所有類別關聯都很薄弱，必須輸出 []

各類別涵蓋的代表主題（僅供判斷參考）：
- 在地關懷：高齡照護、偏鄉教育、青銀共創、原住民族、新住民、社區培力
- 環境永續：淨零碳排、生態保育、氣候變遷、循環經濟、水資源、再生能源
- 健康促進與食品安全：社區健康、食農教育、食品安全、運動促進、慢性病、心理健康
- 產業鏈結與經濟永續：地方創生、產學合作、青年返鄉、社會企業、品牌行銷
- 文化永續：文化保存、傳統技藝、地方文史、原住民族文化、數位典藏
- 其他社會實踐：動物保護、教育平權、數位平權、社會共融、防災教育、人權教育
- 計畫行政：申請、核銷、績效指標、SROI、ESG、USR計畫書
- 轉型正義：白色恐怖、二二八、威權、政治受難者、黨產
- 社會福利：社會救助、長照、家暴防治、街友、弱勢家庭
- 健康醫療：智慧醫療、分級醫療、偏鄉醫療、失智照護、遠距醫療
- 科技研究：AI、半導體、再生醫療、智慧製造、無人機
- 教育創新：STEM、雙語教育、素養導向、實驗教育、服務學習
- 產業經濟：產業競爭力、中小企業、供應鏈、低碳製造、出口貿易
...
_EVAL_RE = re.compile(
    ...
    # A-2 「哪些學校有 + 主觀品質形容詞」（擴大結尾範圍）
    r'|哪.{0,12}(?:完整|完善|健全|完備|齊全|周全|良好|優良|成熟|豐富|積極|全面|深入|扎實|紮實|有效|到位|顯著|實質|具體|落實|真正)(?:的|之).{0,20}'
    # B 成效/影響力判斷
    r'|成效(?:最好|最佳|較好|顯著|卓越|良好)'
    r'|影響力(?:最大|較大|顯著|深遠)'
    r'|哪些.{0,10}(?:真正|實質|具體|有效).{0,10}(?:落實|推動|執行|影響|改變|幫助)'
    r'|有(?:顯著|實質|具體|真實)(?:成效|影響|改變|貢獻)'
    # C 有沒有/是否有 + 主觀詞
    r'|(?:有沒有|是否有|有無).{0,5}(?:完整|完善|健全|良好|優良|有效|顯著|實質|具體)'
    r'|(?:有沒有|是否有|有無).{0,20}(?:機制|制度|配套|支持|規劃)'
    # D 做得好/做得不錯
    r'|做得(?:好|不錯|較好|很好|優秀|出色|紮實|到位)'
    r'|哪.{0,10}做得(?:比較好|更好|較好|不錯|好)'
    r'|在.{2,15}(?:做得好|表現好|表現佳|做得不錯|做得出色)'
    # E 主觀品質形容詞直接觸發
...
題目：「哪些學校有完整的教師升等制度」
---
要評估「完整」的教師升等制度，需先確認您重視的面向：

• 多元升等管道：是否納入教學實務或 USR 社會實踐型升等
• 激勵配套：是否有減授時數或彈性薪資加給
• 職涯保障：是否設有專案教師轉專任的明確機制

請選擇您最在意的角度，我將為您搜尋相關案例。
---

規則：
- 第一行：一句話說明為何需要釐清（針對該問題的主題）
- 列出 2～3 個具體角度
- 最後一行：邀請選擇
- 禁止輸出通用原則、背景說明、一般知識等內容
- 只輸出一個段落結構，不得分成兩部分"""
...
def _school_filter_docs(docs, entry: str, k: int, strict: bool = False):
    """過濾含學校名稱（及計畫名前綴）的 chunk。
    entry 可以是 '學校名' 或 '學校名：計畫名' 格式。
    strict=True：找不到回傳 []，避免 per-plan 列舉時貼上其他學校的內容。
    strict=False（預設）：找不到退回 docs[:k]，適合單校查詢。
    """
    parts = entry.split('：', 1)
    school = parts[0]
    ...
    if plan_prefix:
        ...
        filtered = [d for d in docs if school in src(d)]
    return filtered[:k] if filtered else ([] if strict else docs[:k])
