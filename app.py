...
- 本系統資料庫僅收錄 USR 大學社會責任計畫之成果報告，不涵蓋其他政策、法規、一般知識或非 USR 計畫相關資訊。若問題所求與計畫書內容完全無關，請直接回覆：「本系統僅收錄 USR 計畫成果報告，目前資料庫中沒有關於這個問題的相關資料，建議參考其他管道查詢。」不要嘗試從無關內容中硬拉答案。
...
def ask():
    ...
    def generate():
        try:
            ...
            if len(_in_q_plans) >= 2 and _COUNT_Q_RE.search(question):
            ...
            if len(_in_q_plans) >= 2 and (_GROUP_Q_RE.search(question) or (_llm_is_listing and not _COUNT_Q_RE.search(question))):
