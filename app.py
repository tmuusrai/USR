...
- 若問題明確在**列舉計畫或學校**（如「有哪些計畫」「哪些學校」「列出大學」）：若 context 開頭有【系統強制指令】與【已篩選計畫清單】，必須**嚴格以該清單為唯一依據**，件數、順序均以清單標示為準，不得從【計畫詳細內容】中額外增加或減少條目；若無預建清單，則從 context 中整理所有計畫。輸出格式：以「共X件計畫：」開頭，每條「學校全名：計畫全名」獨立一行，其下空一行寫說明，再空一行接下一條。
